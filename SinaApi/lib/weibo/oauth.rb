  ...
  class OAuth
    ...
    def consumer
      if Weibo::Config.is_proxy_use then
        @consumer ||= ::OAuth::Consumer.new(@ctoken, @csecret, {:site => 'http://api.t.sina.com.cn', :proxy => Weibo::Config.proxy}.merge(consumer_options))
      else
    ...
    def authorize_from_request(request_token, verifier_or_pin)
      access_token = request_token.get_access_token(:oauth_verifier => verifier_or_pin)
      @atoken, @asecret = access_token.token, access_token.secret
    end
